...
/datum/controller/subsystem/vote/proc/get_vote_power(mob/voter)
	var/vote_power = 2 //Caustic Edit - Change default vote power to 2
	if(ishuman(voter))
		var/mob/living/carbon/H = voter
		if(H.stat != DEAD)
			vote_power += 2 //Caustic Edit - Change living vote power to 4
		/*if(H.job)
				...
				if(H.mind)
					for(var/datum/antagonist/D in H.mind.antag_datums)
						if(D.increase_votepwr)
							vote_power += 3*/ //Caustic Edit - Change the vote powers to just a flat 2 for Ghosts, and 4 for living.
