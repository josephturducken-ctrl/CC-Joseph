...
/datum/controller/subsystem/mapping/Initialize(timeofday)
	...
	if(config.defaulted)
		...
		if(!config || config.defaulted)
			to_chat(world, "<span class='boldannounce'>Unable to load next or default map config, defaulting to Vanderlin</span>", MESSAGE_TYPE_OOC)
			config = old_config
	if(map_adjustment)
...
#define INIT_ANNOUNCE(X) to_chat(world, "<span class='boldannounce'>[X]</span>", MESSAGE_TYPE_OOC); log_world(X)
...
/datum/controller/subsystem/mapping/proc/maprotate()
	...
	if (. && VM.map_name != config.map_name)
		to_chat(world, "<span class='boldannounce'>Map rotation has chosen [VM.map_name] for next round!</span>", MESSAGE_TYPE_OOC)
