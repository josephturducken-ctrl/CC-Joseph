...
/proc/build_steward_report_body(list/diff)
	...
	var/list/outpost_manors = diff["outpost_manors"] //CC + TA EDIT
	...
	if(banditry_total > 0)
		...
		for(var/line in banditry_lines)
			body += "&nbsp;&nbsp;- [line]<br>"
		if(diff["outpost_reduction"]) //CC + TA EDIT START
		...
		if(diff["outpost_threat_reduced"])
			body += "<b>Threat relief:</b> [diff["outpost_threat_reduced"]] danger reduced by [diff["outpost_threat_reduction_amount"]] points.<br>" //CC + TA EDIT END
