...
/datum/action/cooldown/spell/vizier/reversion
	...
	name = "Reversion"
	desc = "Marks an adjacent ally's body and position for 15 seconds, allowing them to return to their marked state at will. If the target is under Convergence when activating this, all healing gained after application will remain.<br><br>If used on a target that has died within the last minute, the Vizier may instead rewind their death at the cost of their own Lux. Anything beyond that cannot be rewound."
