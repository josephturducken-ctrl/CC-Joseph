...
/obj/belly/proc/relay_resist(mob/living/living_prey, obj/item/prey_item)
	...
	if(!escapable) //If the stomach has escapable enabled.
		var/struggle_user_message = span_alert(belly_format_string(struggle_messages_inside, living_prey))
...
/obj/belly/proc/resist_default_escape(mob/living/living_prey, obj/item/prey_item)
	var/escape_attempt_owner_message = span_warning(belly_format_string(escape_attempt_messages_owner, living_prey))
	var/escape_attempt_prey_message = span_warning(belly_format_string(escape_attempt_messages_prey, living_prey))
	var/escape_fail_owner_message = span_warning(belly_format_string(escape_fail_messages_owner, living_prey))
	var/escape_fail_prey_message = span_notice(belly_format_string(escape_fail_messages_prey, living_prey))
	...
	if(do_after(living_prey, escapetime, TRUE, owner, target = src))
...
/obj/belly/proc/resist_struggle_outside(mob/living/living_prey)
	var/struggle_outer_message = span_alert(belly_format_string(struggle_messages_outside, living_prey))
	if(private_struggle)
...
/obj/belly/proc/resist_check_escapechance(mob/living/living_prey, obj/item/prey_item)
	...
	if(do_after(living_prey, escapetime, target = src))
		if(escapable && prey_item)
			var/escape_item_owner_message = span_warning(belly_format_string(escape_item_messages_owner, living_prey, item = prey_item))
			var/escape_item_prey_message = span_warning(belly_format_string(escape_item_messages_prey, living_prey, item = prey_item))
			var/escape_item_outside_message = span_warning(belly_format_string(escape_item_messages_outside, living_prey, item = prey_item))
		...
		if(escapable && (living_prey.loc == src) && !living_prey.absorbed) //Does the owner still have escapable enabled?
			var/escape_owner_message = span_warning(belly_format_string(escape_messages_owner, living_prey))
			var/escape_prey_message = span_warning(belly_format_string(escape_messages_prey, living_prey))
			var/escape_outside_message = span_warning(belly_format_string(escape_messages_outside, living_prey))
...
/obj/belly/proc/resist_check_transferchance(mob/living/living_prey, obj/item/prey_item)
	...
	var/primary_transfer_owner_message = span_warning(belly_format_string(primary_transfer_messages_owner, living_prey, dest = transferlocation))
	var/primary_transfer_prey_message = span_warning(belly_format_string(primary_transfer_messages_prey, living_prey, dest = transferlocation))
...
/obj/belly/proc/resist_check_secondary_transferchance(mob/living/living_prey, obj/item/prey_item)
	...
	var/secondary_transfer_owner_message = span_warning(belly_format_string(secondary_transfer_messages_owner, living_prey, dest = transferlocation_secondary))
	var/secondary_transfer_prey_message = span_warning(belly_format_string(secondary_transfer_messages_prey, living_prey, dest = transferlocation_secondary))
...
/obj/belly/proc/resist_check_absorbchance(mob/living/living_prey)
	if(!prob(absorbchance) || digest_mode == DM_ABSORB) //After that, let's have it run the absorb chance.
		return FALSE
	var/absorb_chance_owner_message = span_warning(belly_format_string(absorb_chance_messages_owner, living_prey))
	var/absorb_chance_prey_message = span_warning(belly_format_string(absorb_chance_messages_prey, living_prey))
...
/obj/belly/proc/resist_check_digestchance(mob/living/living_prey)
	if(!prob(digestchance) || digest_mode == DM_DIGEST) //Then, let's see if it should run the digest chance.
		return FALSE
	var/digest_chance_owner_message = span_warning(belly_format_string(digest_chance_messages_owner, living_prey))
	var/digest_chance_prey_message = span_warning(belly_format_string(digest_chance_messages_prey, living_prey))
...
/obj/belly/proc/resist_check_selectchance(mob/living/living_prey)
	if(!prob(selectchance) || digest_mode == DM_SELECT) //Finally, let's see if it should run the selective mode chance.
		return FALSE
	var/select_chance_owner_message = span_warning(belly_format_string(select_chance_messages_owner, living_prey))
	var/select_chance_prey_message = span_warning(belly_format_string(select_chance_messages_prey, living_prey))
...
/obj/belly/proc/relay_absorbed_resist(mob/living/living_prey)
	...
	var/struggle_user_message = span_alert(belly_format_string(absorbed_struggle_messages_inside, living_prey, use_absorbed_count = TRUE))
	to_chat(living_prey, struggle_user_message)

/obj/belly/proc/absorbed_resist_struggle_outside(mob/living/living_prey)
	var/struggle_outer_message = span_alert(belly_format_string(absorbed_struggle_messages_outside, living_prey, use_absorbed_count = TRUE))
	if(private_struggle)
		to_chat(owner, struggle_outer_message)
		return
...
/obj/belly/proc/absorbed_resist_check_escapechance(mob/living/living_prey)
	if(!(prob(escapechance_absorbed) || owner.stat)) //Let's have it check to see if the prey's escape attempt starts.
		return FALSE
	var/escape_attempt_absorbed_owner_message = span_warning(belly_format_string(escape_attempt_absorbed_messages_owner, living_prey))
	var/escape_attempt_absorbed_prey_message = span_warning(belly_format_string(escape_attempt_absorbed_messages_prey, living_prey))
	...
	if(do_after(living_prey, escapetime, target = src))
		if((escapable || owner.stat) && (living_prey.loc == src) && prob(escapechance_absorbed)) //Does the escape attempt succeed?
			var/escape_absorbed_owner_message = span_warning(belly_format_string(escape_absorbed_messages_owner, living_prey))
			var/escape_absorbed_prey_message = span_warning(belly_format_string(escape_absorbed_messages_prey, living_prey))
			var/escape_absorbed_outside_message = span_warning(belly_format_string(escape_absorbed_messages_outside, living_prey))
		...
		var/escape_fail_absorbed_owner_message = span_warning(belly_format_string(escape_fail_absorbed_messages_owner, living_prey))
		var/escape_fail_absorbed_prey_message = span_notice(belly_format_string(escape_fail_absorbed_messages_prey, living_prey))
